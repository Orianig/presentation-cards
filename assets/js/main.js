...
const getInfluencerList = async () => {
  try {
    ...
    influencersList = [data, data, data, data, data, data, data, data];
    // influencer = data
  } catch (error) {
...
const generateInfluencerListHTML = () => {
  ...
  influencersList.map((influencer) => {
    ...
    cardLeftSideHTML.classList.add(
      "d-flex",
      "flex-column",
    ...
    influencerInfoHTML.classList.add(
      "d-flex",
      "flex-column",
      "justify-content-center",
      "align-items-center"
    );
    influencerInfoHTML.innerHTML = `<p class='text-data'><img class='text-icons' src='./assets/img/instagram.png' alt='Icono'>${
      influencer.influcard.username
    }</p><p class='text-data mb-0'>${
      influencer.influcard.gender === 1 ? "Hombre" : "Mujer"
    }, ${influencer.influcard.age} años</p>
        <p class='text-data'><img class='text-icons' src='./assets/img/espana.png' alt='Icono'>${
          influencer.influcard.country === "ES" ? "España" : ""
        }</p><p class='text-data mb-0' >${influencerInterest}...</p>`;
    ...
    const capitalizedName =
      influencer.influcard.name.charAt(0).toUpperCase() +
      influencer.influcard.name.slice(1);

    let cardRightSideHTML = document.createElement("div");
    cardRightSideHTML.classList.add(
      "custom-text",
      "d-flex",
      "flex-column",
      "col-7",
    ...
    audienceHTML.innerHTML = `<i class='fa-solid fa-users col-1'></i><p class='col-6'>Audiencia:</p><p class='col-5 text-end'>${influencer.influcard.followers_formated}</p>`;
    ...
    fakesHTML.innerHTML = `<i class='fa-solid fa-user-xmark col-1'></i><p class='col-6'>Fakes:</p><p class='col-5 text-end'>${formattedFakes}</p>`;

    // Media Eng
    let mediaEngHTML = document.createElement("div");
    mediaEngHTML.classList.add("custom-flex-items");
    mediaEngHTML.innerHTML = `<i class='fa-solid fa-heart col-1'></i><p class='col-6'>Media Eng:</p><p class='col-5 text-end'>${influencer.influcard.avg_engagement_formated}</p>`;

    // Eng Rate
    const engRate = influencer.influcard.er_audiencia;
    const formattedEngRate = engRate.toLocaleString("es-ES", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    });
    ...
    ImpressionsHTML.innerHTML = `<i class='fa-solid fa-eye col-1'></i><p class='col-6'>Impresiones:</p><p class='col-5 text-end'>${influencer.influcard.impressions_formated}</p>`;
